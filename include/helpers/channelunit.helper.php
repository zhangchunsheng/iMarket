...
function GetLink($row) {
	...
	if (preg_match("/^http:|^ftp:/i", $sitepath))
		return $sitepath;
	if($GLOBALS['release']) {
		$cfg_cmspath = "";
	} else {
		$cfg_cmspath = $GLOBALS['cfg_cmspath'];
	}
	$sitepath = str_replace("{cmspath}", $cfg_cmspath, $sitepath);
	$sitepath = preg_replace("/\/{1,}/", "/", $sitepath);
	return $sitepath;
}
...
function GetCacheBlock($cacheid)
{
	...
	if (!file_exists($cachefile) || filesize($cachefile) == 0 || $cfg_puccache_time == 0 || time() - filemtime($cachefile) > $cfg_puccache_time) {
		return '';
	}
