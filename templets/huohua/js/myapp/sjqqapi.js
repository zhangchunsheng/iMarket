...
var QQAPI_UI = {
	...
	UIDEFAULT: function (v, opts) {
		...
		if (v == 0) {
		   ...
		   sjqqapinext.onmouseover=function(){
		   		this.className='sjqqapi_nexthover';
		   }
		   sjqqapinext.onmouseout=function(){
	...
	initPopwinInnerHTML: function (opts) {
		...
		if(opts.type==='soft'){
			popwinInnerHTML += '<em class="sjqqapi_tp lh30 pt20 pb20 fs14" >安装应用前需要在电脑上安装“腾讯手机管家(PC版)”</em>';
		}
		else{
			popwinInnerHTML += '<em class="sjqqapi_tp lh30 pt20 pb20 fs14" >一键下载需在电脑上安装最新版“腾讯手机管家(PC版)”</em>';	
		}
		popwinInnerHTML += '<div class="sjqqapi_con" >';
		popwinInnerHTML += '<img src="'+opts.icon.s+'"  width="40" height="40" class="sjqqapi_icon" id="sjqqapi_icon"/>';
		popwinInnerHTML += '<button class="sjqqapi_downloadapp" id="apidownloadapp"></button>';
		...
		popwinInnerHTML += '<div class="sjqqapi_next" id="sjqqapi_next">立即启动</div>';
		popwinInnerHTML += '<em class="sjqqapi_tp">如果已经安装“腾讯手机管家(PC版)”，请点击</em>';
		popwinInnerHTML += '</div>';
		popwinInnerHTML += '</div>';
		popwinInnerHTML += '</div>';
		...
		popwinInnerHTML += '<div class="clear"></div>';
		popwinInnerHTML += '</div>';
		popwinInnerHTML += '<em class="sjqqapi_tp lh30 pt35" >如果未启动腾讯手机管家(PC版)，请&nbsp;<span class="c1071bb" id="startQQPhone">重试</span>&nbsp;或&nbsp;<span class="c1071bb" onclick="window.location.href=\''+_asistanturl+'\'">重新安装腾讯手机管家</span></em>';
		popwinInnerHTML += '<div class="clear"></div>';
		popwinInnerHTML += '</div>';
		popwinInnerHTML += '<div class="sjqqapi_foot">';
		popwinInnerHTML += '<div class="sjqqapi_next" id="sjqqapi_fin">完&nbsp;&nbsp;成</div>';
		popwinInnerHTML += '<em class="sjqqapi_tp" >部分浏览器会有外部协议请求，请点击[允许]或[启动]</em>';
		popwinInnerHTML += '</div>';
		popwinInnerHTML += '</div>';
		popwinInnerHTML += '</div>';
		if(popdiv==null){
...
var qqapp_dl_apk = function (obj) {//传递 一个dom节点
	...
	var opt = { "url": url,"asistanturlid":asistanturlid, "title": _title, "type": "soft", "api":"qqapp","asistanturl":asistanturl,"icon":appicon,"qq":qq};
...
var qqapp_dl_eb=function(obj){
	...
	var opt = { "url": url,"asistanturlid":asistanturlid, "title": _title, "type": "dlbook", "api":"qqapp","asistanturl":asistanturl,"cardpath":cardpath,"icon":icon};
	QQAPI_UI.initPopwinInnerHTML(opt);
	QQApi.Download(opt);
...
var qqapp_video=function(obj){
	...
	var opt = { "url": url,"asistanturlid":asistanturlid, "title": _title, "type": "video", "api":"qqapp","asistanturl":asistanturl,"cardpath":cardpath};
...
var qqapp_dl_video=function(obj){
	...
	var opt = { "url": url,"asistanturlid":asistanturlid, "title": _title, "type": "dlvideo", "api":"qqapp","asistanturl":asistanturl,"cardpath":cardpath,"icon":icon};
	QQAPI_UI.initPopwinInnerHTML(opt);
	QQApi.Download(opt);
...
var qqapp_dl_magazine=function(obj){
	...
	var opt = { "url": url,"asistanturlid":asistanturlid, "title": _title, "type": "magazine", "api":"qqapp","asistanturl":asistanturl,"cardpath":cardpath,"icon":icon};
	QQAPI_UI.initPopwinInnerHTML(opt);
	QQApi.Download(opt);
...
var qqapp_dl_ring=function(obj){
	...
	var opt = { "url": url,"asistanturlid":asistanturlid, "title": _title, "type": "ring", "api":"qqapp","asistanturl":asistanturl,"cardpath":cardpath,"icon":icon};
	QQAPI_UI.initPopwinInnerHTML(opt);
...
var qqapp_dl_photo=function(obj){
	...
	var opt = { "url": url,"asistanturlid":asistanturlid, "title": _title, "type": "photo", "api":"qqapp","asistanturl":asistanturl,"cardpath":cardpath,"icon":icon};
	QQAPI_UI.initPopwinInnerHTML(opt);
	QQApi.Download(opt);
...
var qqapp_dl_theme=function(obj){
	...
	var opt = { "url": url,"asistanturlid":asistanturlid, "title": _title, "type": "theme", "api":"qqapp","asistanturl":asistanturl,"cardpath":cardpath,"icon":icon};
	QQAPI_UI.initPopwinInnerHTML(opt);
	QQApi.Download(opt);
