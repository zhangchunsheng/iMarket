...
try {
	...
	create_object_by_content($baidu_bcs);
...
function create_object_by_content($baidu_bcs) {
	global $bucket;
	$suffix = "";
	$url = "http://appimg1.3g.qq.com/android/50801/16629897/1.png";
	...
	//$fp = fopen($url, 'rb', 0);
	//echo chunk_split(base64_encode(fread($fp, filesize($url))));
	
	fclose($fp);
	$imageSrc = base64_decode($result);*/
	$imageSrc = file_get_contents($url);
	
	//object name
	$filename = 'foo.' . $suffix;//填入您要保存的名称
